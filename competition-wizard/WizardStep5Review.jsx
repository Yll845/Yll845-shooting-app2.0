...
import { CheckCircle2, AlertTriangle, Calendar, MapPin, Users, Trophy, UserCheck } from 'lucide-react';
import { REQUIRED_OFFICIAL_ROLES } from '@/lib/constants';

export default function WizardStep5Review({ basics, events, relays, officials }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display font-semibold text-lg">Hapi 5 – Verifikimi Final</h2>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Card className="border shadow-sm">
          <CardContent className="pt-4 space-y-2">
            <div className="flex items-center gap-2 text-sm font-semibold"><Trophy className="h-4 w-4 text-primary" />Eventet ({events.length})</div>
            {events.length === 0 ? (
              <p className="text-sm text-destructive">⚠️ Nuk ka evente!</p>
            ) : (
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {events.map((ev, i) => (
                  <div key={i} className="flex gap-1 flex-wrap">
                    <Badge variant="outline" className="text-[10px]">{ev.discipline}</Badge>
                    <Badge className="bg-blue-100 text-blue-800 text-[10px]">{ev.gender}</Badge>
                    <Badge className="bg-green-100 text-green-800 text-[10px]">{ev.age_category}</Badge>
                  </div>
                ))}
              </div>
            )}
